...
+ (SVProgressHUD*)sharedView {
    ...
    sharedView.frame = [[UIScreen mainScreen] bounds];//配合changeDistance函数
    return sharedView;
}
...
//此函数可以控制出现的y值 By Aevit
+ (void)changeDistance:(CGFloat)dist {
    CGRect currFrame = [SVProgressHUD sharedView].frame;
    currFrame.origin.y += dist;
    [SVProgressHUD sharedView].frame = currFrame;
}
...
+ (void)showSuccessWithStatusNoAutoDismiss:(NSString *)string {
    [SVProgressHUD showImage:[UIImage imageNamed:@"SVProgressHUD.bundle/success.png"] status:string isAutoDismiss:NO];
}

+ (void)showErrorWithStatusNoAutoDismiss:(NSString *)string {
    [SVProgressHUD showImage:[UIImage imageNamed:@"SVProgressHUD.bundle/error.png"] status:string isAutoDismiss:NO];
}
...
+ (void)showImage:(UIImage *)image status:(NSString *)string isAutoDismiss:(BOOL)_isAutoDismiss {
    [[SVProgressHUD sharedView] showImage:image status:string duration:0.0f];
}
...
//        CGSize stringSize = [string sizeWithFont:self.stringLabel.font constrainedToSize:CGSizeMake(200, 300)];
        ...
        NSDictionary *attribute = @{NSFontAttributeName: self.stringLabel.font};
...
//    [self.hudView addSubview:button];
